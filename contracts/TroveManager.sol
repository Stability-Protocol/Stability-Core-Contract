...
contract TroveManager is TroveManagerBase, ITroveManager, ReentrancyGuard {
    ...
    */
    ...
    mapping (address => uint) public L_SUSDDebt;
    ...
    struct RewardSnapshot {
        ...
        mapping(address => uint) SUSDDebts;
    }
    ...
    mapping (address => uint) public lastSUSDDebtError_Redistribution;
    ...
    **/
    ...
    event LTermsUpdated(address _Coll_Address, uint _L_Coll, uint _L_SUSDDebt);
    ...
    */
    ...
    function movePendingTroveRewardsToActivePool(IActivePool _activePool, IDefaultPool _defaultPool, uint _SUSD, address[] memory _tokens, uint[] memory _amounts, address _borrower) external override {
        _requireCallerIsTML();
        _movePendingTroveRewardsToActivePool(_activePool, _defaultPool, _SUSD, _tokens, _amounts, _borrower);
    }

    function _movePendingTroveRewardsToActivePool(IActivePool _activePool, IDefaultPool _defaultPool, uint _SUSD, address[] memory _tokens, uint[] memory _amounts, address _borrower) internal {
        _defaultPool.decreaseSUSDDebt(_SUSD);
        _activePool.increaseSUSDDebt(_SUSD);
    ...
    */
    ...
    function getCurrentICR(address _borrower) external view override returns (uint ICR) {
        (newColls memory colls, uint currentSUSDDebt) = _getCurrentTroveState(_borrower);

        ICR = _getICRColls(colls, currentSUSDDebt);
    }
    ...
    function getCurrentRICR(address _borrower) external view override returns (uint RICR) {
        ...
        RICR = _getRICRColls(colls, currentSUSDDebt);
    }

    // Gets current trove state as colls and debt. 
    function _getCurrentTroveState(address _borrower) internal view
    returns (newColls memory colls, uint SUSDdebt) {
        newColls memory pendingCollReward = _getPendingCollRewards(_borrower);
        uint pendingSUSDDebtReward = getPendingSUSDDebtReward(_borrower);
        
        SUSDdebt = Troves[_borrower].debt.add(pendingSUSDDebtReward);
    ...
    function _applyPendingRewards(IActivePool _activePool, IDefaultPool _defaultPool, address _borrower) internal {
        if (hasPendingRewards(_borrower)) {
            ...
            Troves[_borrower].debt = Troves[_borrower].debt.add(pendingSUSDDebtReward);

            _updateTroveRewardSnapshots(_borrower);

            // Transfer from DefaultPool to ActivePool
            _movePendingTroveRewardsToActivePool(_activePool, _defaultPool, pendingSUSDDebtReward, pendingCollReward.tokens, pendingCollReward.amounts, _borrower);
    ...
    function _updateTroveRewardSnapshots(address _borrower) internal {
        ...
        for (uint256 i; i < allCollsLen; ++i) {
            ...
            rewardSnapshots[_borrower].SUSDDebts[asset] = L_SUSDDebt[asset];
        }
    ...
    function getPendingSUSDDebtReward(address _borrower) public view override returns (uint pendingSUSDDebtReward) {
        if (Troves[_borrower].status != Status.active) {
            return 0;
        }
        address[] memory allColls = Troves[_borrower].colls.tokens;
        ...
        for (uint256 i; i < allCollsLen; ++i) {
            address coll = allColls[i];
            uint snapshotSUSDDebt = rewardSnapshots[_borrower].SUSDDebts[coll];
            uint rewardPerUnitStaked = L_SUSDDebt[allColls[i]].sub(snapshotSUSDDebt);
            if ( rewardPerUnitStaked == 0) { continue; }

            uint stake =  Troves[_borrower].stakes[coll];

            uint assetSUSDDebtReward = stake.mul(rewardPerUnitStaked).div(DECIMAL_PRECISION);
            pendingSUSDDebtReward = pendingSUSDDebtReward.add(assetSUSDDebtReward);
        }
    }
    ...
    returns (uint, address[] memory, uint[] memory, uint, address[] memory, uint[] memory)
    {
        ...
        debt = debt.add(pendingSUSDDebtReward);

        // add in pending rewards to colls
        colls = _sumColls(colls, pendingCollReward);

        return (debt, colls.tokens, colls.amounts, pendingSUSDDebtReward, pendingCollReward.tokens, pendingCollReward.amounts);
    }
    ...
    function redistributeDebtAndColl(IActivePool _activePool, IDefaultPool _defaultPool, uint _debt, address[] memory _tokens, uint[] memory _amounts) external override {
        ...
        */
        ...
        for (uint256 i; i < tokensLen; ++i) {
            ...
            uint SUSDDebtNumerator = proratedDebtForCollateral.mul(DECIMAL_PRECISION).add(lastSUSDDebtError_Redistribution[token]);
            if (totalStakes[token] != 0) {
                ...
                uint SUSDDebtRewardPerUnitStaked = SUSDDebtNumerator.div(thisTotalStakes.mul(10 ** (18 - dec)));

                lastCollError_Redistribution[token] = CollNumerator.sub(CollRewardPerUnitStaked.mul(thisTotalStakes));
                lastSUSDDebtError_Redistribution[token] = SUSDDebtNumerator.sub(SUSDDebtRewardPerUnitStaked.mul(thisTotalStakes.mul(10 ** (18 - dec))));

                // Add per-unit-staked terms to the running totals
                L_Coll[token] = L_Coll[token].add(CollRewardPerUnitStaked);
                L_SUSDDebt[token] = L_SUSDDebt[token].add(SUSDDebtRewardPerUnitStaked);
                emit LTermsUpdated(token, L_Coll[token], L_SUSDDebt[token]);
            }
        }

        // Transfer coll and debt from ActivePool to DefaultPool
        _activePool.decreaseSUSDDebt(_debt);
        _defaultPool.increaseSUSDDebt(_debt);
    ...
    function _closeTrove(address _borrower, Status closedStatus) internal {
        ...
        for (uint256 i; i < allCollsLen; ++i) {
            ...
            rewardSnapshots[_borrower].SUSDDebts[thisAllColls] = 0;
        }
    ...
    */
    ...
    function _getRedemptionFee(uint _SUSDRedeemed) internal view returns (uint) {
        return _calcRedemptionFee(getRedemptionRate(), _SUSDRedeemed);
    }

    function getRedemptionFeeWithDecay(uint _SUSDRedeemed) external view override returns (uint) {
        return _calcRedemptionFee(getRedemptionRateWithDecay(), _SUSDRedeemed);
    }

    function _calcRedemptionFee(uint _redemptionRate, uint _SUSDRedeemed) internal pure returns (uint) {
        uint redemptionFee = _redemptionRate.mul(_SUSDRedeemed).div(DECIMAL_PRECISION);
        require(redemptionFee < _SUSDRedeemed, "TM:Fee>returned colls");
    ...
    function getBorrowingFee(uint _SUSDDebt) external view override returns (uint) {
        return _calcBorrowingFee(getBorrowingRate(), _SUSDDebt);
    }

    function getBorrowingFeeWithDecay(uint _SUSDDebt) external view override returns (uint) {
        return _calcBorrowingFee(getBorrowingRateWithDecay(), _SUSDDebt);
    }

    function _calcBorrowingFee(uint _borrowingRate, uint _SUSDDebt) internal pure returns (uint) {
        return _borrowingRate.mul(_SUSDDebt).div(DECIMAL_PRECISION);
    }
    ...
    function getL_SUSD(address _token) external view override returns (uint) {
        return L_SUSDDebt[_token];
    }
    ...
    function getRewardSnapshotSUSD(address _borrower, address _token) external view override returns (uint) {
        return rewardSnapshots[_borrower].SUSDDebts[_token];
    }
    ...
    function getCurrentTroveState(address _borrower) external override view returns (address[] memory, uint[] memory, uint) {
        (newColls memory colls, uint currentSUSDDebt) = _getCurrentTroveState(_borrower);
        return (colls.tokens, colls.amounts, currentSUSDDebt);
    }
    ...
    function susdToken() external view override returns (ISUSDToken) {
        return susdTokenContract;
    }

    function stbToken() external view override returns (ISTBToken) {
        return stbTokenContract;
    }

    function sSTB() external view override returns (ISSTB) {
        return sSTBContract;
    }
 }