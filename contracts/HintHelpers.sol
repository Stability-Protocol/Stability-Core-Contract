...
// TODO 
    function getRedemptionHints(
        ...
        returns (
            ...
            uint truncatedSUSDamount
        )
    {
        ISortedTroves sortedTrovesCached = sortedTroves;

        uint remainingSUSD = _SUSDamount;
        ...
        while (currentTroveuser != address(0) && remainingSUSD != 0 && _maxIterations-- != 0) {
            uint netSUSDDebt = _getNetDebt(troveManager.getTroveDebt(currentTroveuser))
                .add(troveManager.getPendingSUSDDebtReward(currentTroveuser));

            if (netSUSDDebt > remainingSUSD) { // Partial redemption
                if (netSUSDDebt > MIN_NET_DEBT) { // MIN NET DEBT = 1800
                    uint maxRedeemableSUSD = LiquityMath._min(remainingSUSD, netSUSDDebt.sub(MIN_NET_DEBT));

                    uint newColl = _calculateVCAfterRedemption(currentTroveuser, maxRedeemableSUSD);
                    uint newDebt = netSUSDDebt.sub(maxRedeemableSUSD);

                    uint compositeDebt = _getCompositeDebt(newDebt);
                    partialRedemptionHintICR = LiquityMath._computeCR(newColl, compositeDebt);

                    remainingSUSD = remainingSUSD.sub(maxRedeemableSUSD);
                }
                break;
            } else { // Full redemption in this case
                remainingSUSD = remainingSUSD.sub(netSUSDDebt);
            }

            currentTroveuser = sortedTrovesCached.getPrev(currentTroveuser);
        }

        truncatedSUSDamount = _SUSDamount.sub(remainingSUSD);
    }
    ...
    function _calculateVCAfterRedemption(address _borrower, uint _SUSDAmount) internal view returns (uint newCollVC) {
        ...
        uint baseLot = _SUSDAmount.mul(DECIMAL_PRECISION);
