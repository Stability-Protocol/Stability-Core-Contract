...
/*
 * The Stability Pool holds SUSD tokens deposited by Stability Pool depositors.
 *
 * When a trove is liquidated, then depending on system conditions, some of its SUSD debt gets offset with
...
contract StabilityPool is PoolBase, Ownable, CheckContract, IStabilityPool {
    ...
    uint256 internal totalSUSDDeposits;
    ...
    uint256 public lastSTBError;
    // Error trackers for the error correction in the offset calculation
    uint256[] public lastAssetError_Offset;
    uint256 public lastSUSDLossError_Offset;
    ...
    event StabilityPoolSUSDBalanceUpdated(uint256 _newBalance);
    ...
    event STBPaidToDepositor(address indexed _depositor, uint256 _STB);
    event STBPaidToFrontEnd(address indexed _frontEnd, uint256 _STB);
    ...
    function setAddresses(
        ...
        checkContract(_susdTokenAddress);
    ...
    function getTotalSUSDDeposits() external view override returns (uint256) {
        return totalSUSDDeposits;
    }
    ...
    function provideToSP(uint256 _amount, address _frontEndTag) external override {
        ...
        _triggerSTBIssuance(communityIssuanceCached);

        if (initialDeposit == 0) {
            _setFrontEndTag(msg.sender, _frontEndTag);
        }
        (address[] memory assets, uint256[] memory amounts) = getDepositorGains(msg.sender);
        uint256 compoundedSUSDDeposit = getCompoundedSUSDDeposit(msg.sender);
        uint256 SUSDLoss = initialDeposit.sub(compoundedSUSDDeposit); // Needed only for event log

        // First pay out any YETI gains
        address frontEnd = deposits[msg.sender].frontEndTag;
        _payOutSTBGains(communityIssuanceCached, msg.sender, frontEnd);
        ...
        // and throws an event
        _sendSUSDtoStabilityPool(msg.sender, _amount);

        uint256 newDeposit = compoundedSUSDDeposit.add(_amount);
        _updateDepositAndSnapshots(msg.sender, newDeposit);
        emit UserDepositChanged(msg.sender, newDeposit);

        emit GainsWithdrawn(msg.sender, assets, amounts, SUSDLoss); // YUSD Loss required for event log
    ...
    function withdrawFromSP(uint256 _amount) external override {
        ...
        uint256 SUSDtoWithdraw = LiquityMath._min(_amount, compoundedSUSDDeposit);
        uint256 SUSDLoss = initialDeposit.sub(compoundedSUSDDeposit); // Needed only for event log
        ...
        // Update front end stake
        uint256 compoundedFrontEndStake = getCompoundedFrontEndStake(frontEnd);
        uint256 newFrontEndStake = compoundedFrontEndStake.sub(SUSDtoWithdraw);
        _updateFrontEndStakeAndSnapshots(frontEnd, newFrontEndStake);
        emit FrontEndStakeChanged(frontEnd, newFrontEndStake, msg.sender);

        _sendSUSDToDepositor(msg.sender, SUSDtoWithdraw);

        // Update deposit
        uint256 newDeposit = compoundedSUSDDeposit.sub(SUSDtoWithdraw);
        _updateDepositAndSnapshots(msg.sender, newDeposit);
        emit UserDepositChanged(msg.sender, newDeposit);
    ...
    function _triggerSTBIssuance(ICommunityIssuance _communityIssuance) internal {
        uint256 STBIssuance = _communityIssuance.issueSTB();
        _updateG(STBIssuance);
    }

    function _updateG(uint256 _STBIssuance) internal {
        uint256 totalSUSD = totalSUSDDeposits; // cached to save an SLOAD
        /*
         * When total deposits is 0, G is not updated. In this case, the YETI issued can not be obtained by later
         * depositors - it is missed out on, and remains in the balanceof the CommunityIssuance contract.
         *
         */
        if (totalSUSD == 0 || _STBIssuance == 0) {
            return;
        }

        uint256 STBPerUnitStaked = _computeSTBPerUnitStaked(_STBIssuance, totalSUSD);

        uint256 marginalSTBGain = STBPerUnitStaked.mul(P);
    ...
    function _computeSTBPerUnitStaked(uint256 _STBIssuance, uint256 _totalSUSDDeposits)
        internal
        returns (uint256)
    {
        /*
         ...
         */
        uint256 STBNumerator = _STBIssuance.mul(DECIMAL_PRECISION).add(lastSTBError);

        uint256 STBPerUnitStaked = STBNumerator.div(_totalSUSDDeposits);
        lastSTBError = STBNumerator.sub(STBPerUnitStaked.mul(_totalSUSDDeposits));

        return STBPerUnitStaked;
    }
    ...
    function offset(
        ...
        _requireCallerIsTML();
        uint256 totalSUSD = totalSUSDDeposits; // cached to save an SLOAD
        if (totalSUSD == 0 || _debtToOffset == 0) {
            return;
        }

        _triggerSTBIssuance(communityIssuance);

        (
            uint256[] memory AssetGainPerUnitStaked,
            uint256 SUSDLossPerUnitStaked
        ) = _computeRewardsPerUnitStaked(_tokens, _amountsAdded, _debtToOffset, totalSUSD);

        _updateRewardSumAndProduct(_tokens, AssetGainPerUnitStaked, SUSDLossPerUnitStaked); // updates S and P
    ...
    */
    function _computeRewardsPerUnitStaked(
        ...
        uint256[] memory _amountsAdded,
        uint256 _debtToOffset,
        uint256 _totalSUSDDeposits
    ) internal returns (uint256[] memory AssetGainPerUnitStaked, uint256 SUSDLossPerUnitStaked) {
        ...
        require(_debtToOffset <= _totalSUSDDeposits, "SP:This debt less than totalSUSD");
        if (_debtToOffset == _totalSUSDDeposits) {
            SUSDLossPerUnitStaked = DECIMAL_PRECISION; // When the Pool depletes to 0, so does each deposit
            lastSUSDLossError_Offset = 0;
        } else {
            uint256 SUSDLossNumerator = _debtToOffset.mul(DECIMAL_PRECISION).sub(
                lastSUSDLossError_Offset
            );
            /*
             ...
             */
            SUSDLossPerUnitStaked = (SUSDLossNumerator.div(_totalSUSDDeposits)).add(1);
            lastSUSDLossError_Offset = (SUSDLossPerUnitStaked.mul(_totalSUSDDeposits)).sub(
                SUSDLossNumerator
            );
        }
        ...
        for (uint256 i; i < amountsLen; ++i) {
            AssetGainPerUnitStaked[i] = CollateralNumerators[i].mul(currentP).div(_totalSUSDDeposits);
        }

        for (uint256 i; i < amountsLen; ++i) {
            uint256 tokenIDX = whitelist.getIndex(_tokens[i]);
            lastAssetError_Offset[tokenIDX] = CollateralNumerators[i].sub(
                AssetGainPerUnitStaked[i].mul(_totalSUSDDeposits).div(currentP)
            );
        }
    ...
    function _updateRewardSumAndProduct(
        ...
        uint256 _SUSDLossPerUnitStaked
    ) internal {
        uint256 currentP = P;
        uint256 newP;

        require(_SUSDLossPerUnitStaked <= DECIMAL_PRECISION, "SP: SUSDLoss < 1");
        /*
         ...
         */
        uint256 newProductFactor = uint256(DECIMAL_PRECISION).sub(_SUSDLossPerUnitStaked);
    ...
    function _moveOffsetCollAndDebt(
        ...
        activePoolCached.decreaseSUSDDebt(_debtToOffset);
        _decreaseSUSD(_debtToOffset);

        // Burn the debt that was successfully offset
        susdToken.burn(address(this), _debtToOffset);

        activePoolCached.sendCollaterals(address(this), _collsToAdd, _amountsToAdd);
    }

    // Decreases YUSD Stability pool balance. 
    function _decreaseSUSD(uint256 _amount) internal {
        uint256 newTotalSUSDDeposits = totalSUSDDeposits.sub(_amount);
        totalSUSDDeposits = newTotalSUSDDeposits;
        emit StabilityPoolSUSDBalanceUpdated(newTotalSUSDDeposits);
    }
     ...
     * d0 is the last recorded deposit value.
     */
    function getDepositorSTBGain(address _depositor) public view override returns (uint256) {
        uint256 initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) {
            return 0;
        }
        ...
        uint256 STBGain = kickbackRate
            .mul(_getSTBGainFromSnapshots(initialDeposit, snapshots))
     ...
     */
    function getFrontEndSTBGain(address _frontEnd) public view override returns (uint256) {
        ...
        uint256 STBGain = frontEndShare
            .mul(_getSTBGainFromSnapshots(frontEndStake, snapshots))
            .div(DECIMAL_PRECISION);
        return STBGain;
    }

    function _getSTBGainFromSnapshots(uint256 initialStake, Snapshots storage snapshots)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 STBGain = initialStake.mul(firstPortion.add(secondPortion)).div(P_Snapshot).div(
            DECIMAL_PRECISION
        );
     ...
     */
    function getCompoundedSUSDDeposit(address _depositor) public view override returns (uint256) {
        uint256 initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) {
            return 0;
        }
    ...
    function _sendSUSDtoStabilityPool(address _address, uint256 _amount) internal {
        susdToken.sendToPool(_address, address(this), _amount);
        uint256 newTotalSUSDDeposits = totalSUSDDeposits.add(_amount);
        totalSUSDDeposits = newTotalSUSDDeposits;
        emit StabilityPoolSUSDBalanceUpdated(newTotalSUSDDeposits);
    }
    ...
    function _sendSUSDToDepositor(address _depositor, uint256 SUSDWithdrawal) internal {
        if (SUSDWithdrawal == 0) {
            return;
        }

        susdToken.returnFromPool(address(this), _depositor, SUSDWithdrawal);
        _decreaseSUSD(SUSDWithdrawal);
    }
    ...
    function _payOutSTBGains(
        ...
        if (_frontEnd != address(0)) {
            uint256 frontEndSTBGain = getFrontEndSTBGain(_frontEnd);
            _communityIssuance.sendSTB(_frontEnd, frontEndSTBGain);
            emit STBPaidToFrontEnd(_frontEnd, frontEndSTBGain);
        }

        // Pay out depositor's YETI gain
        uint256 depositorSTBGain = getDepositorSTBGain(_depositor);
        _communityIssuance.sendSTB(_depositor, depositorSTBGain);
        emit STBPaidToDepositor(_depositor, depositorSTBGain);
    }
