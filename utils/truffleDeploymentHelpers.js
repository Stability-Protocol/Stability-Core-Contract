...
const PriceFeedTestnet = artifacts.require("./PriceFeed.sol")
const SUSDToken = artifacts.require("./SUSDToken.sol")
...
const deployLiquity = async () => {
  const priceFeedTestnet = await PriceFeed.new()
...
const getAddresses = (contracts) => {
  return {
    ...
    SUSDToken: contracts.susdToken.address,
...
const connectContracts = async (contracts, addresses) => {
  ...
  await contracts.troveManager.setSUSDToken(addresses.SUSDToken)
