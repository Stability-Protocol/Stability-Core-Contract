...
*
*/
contract STBToken is ISTBToken {
    using SafeMath for uint256;

    // --- ERC20 Data ---

    string constant internal _NAME = "Stability Protocol";
    string constant internal _SYMBOL = "STB";
    ...
    address public immutable sSTBAddress;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        _approve(sender, msg.sender, _allowances[sender][msg.sender].sub(amount, "STB: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) external override returns (bool) {
        _approve(msg.sender, spender, _allowances[msg.sender][spender].sub(subtractedValue, "STB: decreased allowance below zero"));
        return true;
    }

    function sendToSSTB(address _sender, uint256 _amount) external override {
        _requireCallerIsSSTB();
        _transfer(_sender, sSTBAddress, _amount);
    }
    ...
    override
    {
        require(deadline >= block.timestamp, 'STB: expired deadline');
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        require(sender != address(0), "STB: transfer from the zero address");

        _balances[sender] = _balances[sender].sub(amount, "STB: transfer amount exceeds balance");
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "STB: Cannot transfer tokens directly to the STB token contract"
        );
    }

    function _requireCallerIsSYETI() internal view {
        require(msg.sender == sSTBAddress, "STB: caller must be the SSTB contract");
    }
