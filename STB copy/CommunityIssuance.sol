...
contract CommunityIssuance is ICommunityIssuance, Ownable, CheckContract, BaseMath {
    ...
    */
    uint constant public STBSupplyCap = 32e24; // 32 million

    IYETIToken public stbToken;

    address public stabilityPoolAddress;

    uint public totalSTBIssued;
        ...
        override 
    {
        ...
        stbToken = ISTBToken(_stbTokenAddress);
        stabilityPoolAddress = _stabilityPoolAddress;

        // When YETIToken deployed, it should have transferred CommunityIssuance's YETI entitlement
        uint STBBalance = stbToken.balanceOf(address(this));
        require(STBBalance >= STBSupplyCap, "setAddresses: balance must be less than supplycap");
    ...
    function issueSTB() external override returns (uint) {
        _requireCallerIsStabilityPool();

        uint latestTotalSTBIssued = STBSupplyCap.mul(_getCumulativeIssuanceFraction()).div(DECIMAL_PRECISION);
        uint issuance = latestTotalSTBIssued.sub(totalSTBIssued);

        totalSTBIssued = latestTotalSTBIssued;
        emit TotalSTBIssuedUpdated(latestTotalSTBIssued);
    ...
    function sendSTB(address _account, uint _STBamount) external override {
        _requireCallerIsStabilityPool();

        stbToken.safeTransfer(_account, _STBamount);
    }
