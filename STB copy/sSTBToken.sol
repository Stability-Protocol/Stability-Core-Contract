...
contract sSTBToken is IERC20, Domain, BoringOwnable {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    using BoringERC20 for IERC20;

    string public constant symbol = "sSTB";
    string public constant name = "Staked Stability Tokens";
    uint8 public constant decimals = 18;
    uint256 public override totalSupply;
    uint256 private constant LOCK_TIME = 69 hours;
    uint256 public effectiveStbTokenBalance;
    ...
    event BuyBackExecuted(uint SUSDToSell, uint amounts0, uint amounts1);
    event Rebase(uint additionalStbTokenBalance);

    function balanceOf(address user) public view override returns (uint256) {
        return users[user].balance;
    }

    function setAddresses(ISTBToken _stb, IERC20 _susd) external onlyOwner {
    ...
    function mint(uint256 amount) public returns (bool) {
        ...
        stbToken.sendToSYETI(msg.sender, amount);
        effectiveStbTokenBalance = effectiveStbTokenBalance.add(amount);
    ...
    function _burn(
        ...
        uint256 amount = (shares * effectiveStbTokenBalance) / totalSupply;
        users[from].balance = user.balance.sub(shares.to128()); // Must check underflow
        totalSupply -= shares;

        stbToken.transfer(to, amount);
        effectiveStbTokenBalance = effectiveStbTokenBalance.sub(amount);
     ...
     */
    function buyBack(address _routerAddress, uint256 _SUSDToSell, uint256 _STBOutMin) external onlyOwner {
        require(_SUSDToSell != 0, "Zero amount");
        require(susdToken.balanceOf(address(this)) >= _SUSDToSell, "Not enough SUSD in contract");
        _buyBack(_routerAddress, _SUSDToSell, _STBOutMin);
    }
    ...
    function publicBuyBack(address _routerAddress) external {
        uint256 SUSDBalance = susdToken.balanceOf(address(this));
        require(SUSDBalance != 0, "No SUSD in contract");
        ...
        uint256 SUSDToSell = div(SUSDBalance.mul(5), 100);
        _buyBack(_routerAddress, SUSDToSell, 0);
    }

    // Internal function calls the router function for buyback and emits event with amount of YETI bought and YUSD spent. 
    function _buyBack(address _routerAddress, uint256 _SUSDToSell, uint256 _STBOutMin) internal {
        // Checks internal mapping to see if router is valid
        require(validRouters[_routerAddress] == true, "Invalid router passed in");
        require(susdToken.approve(_routerAddress, 0));
        require(susdToken.increaseAllowance(_routerAddress, _YUSDToSell));
        lastBuybackTime = block.timestamp;
        uint256[] memory amounts = IsSTBRouter(_routerAddress).swap(_SUSDToSell, _STbOutMin, address(this));
        ...
        emit BuyBackExecuted(_SUSDToSell, amounts[0], amounts[1]);
    }
    ...
    function rebase() external {
        ...
        uint256 stbTokenBalance = stbToken.balanceOf(address(this));  
        // amount of YETI free / available to give out
        uint256 adjustedStbTokenBalance = stbTokenBalance.sub(effectiveStbTokenBalance); 
        // in YETI, amount that should be eligible to give out.
        uint256 valueOfContract = _getValueOfContract(adjustedStbTokenBalance); 
        // in YETI, amount to rebase
        uint256 amountStbToRebase = div(valueOfContract.mul(transferRatio), 1e18); 
        ...
        if (amountStbToRebase > adjustedStbTokenBalance) {
            amountStbToRebase = adjustedStbTokenBalance;
        }
        // rebase amount joins the effective supply. 
        effectiveStbTokenBalance = effectiveStbTokenBalance.add(amountStbToRebase);
        // update rebase time
        lastRebaseTime = block.timestamp;
        emit Rebase(amountStbToRebase);
    }
    ...
    function _getValueOfContract(uint _adjustedStbTokenBalance) internal view returns (uint256) {
        uint256 susdTokenBalance = susdToken.balanceOf(address(this));
        return div(susdTokenBalance.mul(1e18), lastBuybackPrice).add(_adjustedStbTokenBalance);
    }
